...
const uniq = (array, callback) => {
  ...
  for (let i = -1; i < array.length; i++){
    for (let j = 0; j < array.length; j++){
      if (array[i] != array[j] && uniqArr.includes(array[j]) == false){
...
each (names, eachName = (item, index) => {
...
const addingFactory = x => {return innerFunct = y => x + y}
...
let addTwentyFour = addingFactory(24)

console.log(addTwentyFour(24))