...
const evenNumbers = mixedNumbers.filter(evenNums => evenNums % 2 == 0)
...
// CODE HERE
const postTaxPrices = prices.map(taxNums => taxNums * 1.07)
...
// CODE HERE
const totalPop = 0;
const totalPopulation = populations.reduce((prevPop, currentPop, totalPop) => prevPop + currentPop, totalPop)
...
// CODE HERE
const myStrongest = monstersInYourPocket.filter(myStrongest => myStrongest["CP"] > 200)
...
const ordersTotal = orders.map(ordersMath => ordersMath["price"] * (ordersMath["tax"] + 1))
...
const bobsPurchases = purchases.filter(person => person["owner"] == "Bob")
let totalPurchases = 0
bobsPurchases.forEach(price => totalPurchases += price["price"])
